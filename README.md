...
- insert:
    - id: tg-bridge
      ...
      config:
        ...
        restartScript: '/root/start-dsh-web.sh'  # 可选：/restart 用的重启脚本
```
...
export TG_RESTART_SCRIPT='/root/start-dsh-web.sh'  # 可选
dsh web   # 或你的启动脚本
```
...
| `restartScript` | `TG_RESTART_SCRIPT` | | 空 | `/restart` 用的重启脚本；不配则命令不可用 |
...
| `/restart` | 远程重启 DSH web（需配置 restartScript；重启后自动汇报状态） |
| `/help` | 命令列表 |
...
3. **`/restart` 显示"未配置 restartScript"**：在 patch 或环境变量里配置 `restartScript` 指向部署自己的重启脚本。
