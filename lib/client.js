    ...
    function TgBridgeCard(props) {
      ...
      function save() {
        ...
        requestConfig("POST", { patch: patch }).then(function (res) {
          if (res.ok) {
            setDirty(false);
            // Immediately mask the token again so the secret never lingers in
            // the field after save (reopening/refresh shows the placeholder).
      ...
      if (values !== null) {
        rows = FIELDS.map(function (field) {
          ...
          // Password fields: plaintext only WHILE the user is typing a new
          // value (convenience to verify the input); a set token shows the
          // masked placeholder and empty value, and after save the value is
          // reset to the mask so reopening always hides the secret.
          var isTokenTyping = isPassword && current !== undefined && current !== "" && current !== "••••••••";
          return React.createElement("div", { style: rowStyle, key: field.key },
            React.createElement("label", { htmlFor: "tg-" + field.key, style: labelStyle }, label),
            React.createElement("input", {
              id: "tg-" + field.key,
              type: isTokenTyping ? "text" : (field.type === "password" ? "password" : (field.type === "number" ? "number" : "text")),
              style: inputStyle,
              value: isPassword ? (isTokenTyping ? String(current) : "") : (current === undefined ? "" : String(current)),
