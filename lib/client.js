...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var FIELDS = [
      ...
      { key: "allowedChat", type: "text", labelKey: "fieldAllowedChat" },
    ...
    var zh = {
      ...
      placeholderToken: "留空保持不变",
      tokenSet: "（已设置）",
      fieldAllowedChat: "允许的 Chat ID",
      fieldAllowedUsers: "允许的用户/群组（每行: chatId [label]）",
      placeholderUsers: "123456789 user1\n-1001234567890 group",
    ...
    var en = {
      ...
      fieldBotToken: "Bot Token",
      placeholderToken: "Leave blank to keep",
      tokenSet: " (set)",
      fieldAllowedChat: "Allowed Chat ID",
      fieldAllowedUsers: "Allowed users/groups (one per line: chatId [label])",
      placeholderUsers: "123456789 user1\n-1001234567890 group",
    ...
    function TgBridgeCard(props) {
      ...
      function setField(key) {
        return function (e) {
          ...
          setValues(function (prev) {
            ...
            } else if (field && field.type === "chatlines") {
              var users = parseChatLines(raw);
              next[key] = users.length ? users : undefined;
            } else if (field && field.type === "chatids") {
              var ids = parseIdLines(raw);
              next[key] = ids.length ? ids : undefined;
      ...
      function save() {
        ...
        for (var i = 0; i < FIELDS.length; i++) {
          ...
          if (v !== undefined && v !== "" && v !== "••••••••") patch[field.key] = v;
        }
      ...
      if (values !== null) {
        rows = FIELDS.map(function (field) {
          ...
          if (field.type === "chatlines" || field.type === "chatids") {
            var text = field.type === "chatlines" ? formatChatLines(current) : formatIdLines(current);
          ...
          return React.createElement("div", { style: rowStyle, key: field.key },
            ...
            React.createElement("input", {
              ...
              placeholder: isPassword ? t("placeholderToken") : "",
              onChange: setField(field.key)
            })
          );
        });
