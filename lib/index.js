...
function apply(ctx, config = {}) {
  ...
  const build = () => {
    ...
    try { bridge?.stop(); } catch {}
    // Ensure the settings service is resolved even if the webServer inject has
    // not fired yet on this boot order (it can run after the first build).
  ...
  ctx.inject(["webServer"], (webCtx) => {
    ...
    // the GUI edits — one source of truth.
    settingsService = webCtx.get("settings");
    try { bridge?.attachHost({ permissionPresets, sessionService, settingsService }); } catch {}
