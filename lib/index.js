...
const DEFAULTS = {
  ...
  // restartScript is intentionally unset by default: it points at the
  // deployment's own restart script, which only the operator knows.
  restartScript: "",
...
function envOverrides() {
  ...
  s("TG_MUX_URL", "muxUrl");
  s("TG_RESTART_SCRIPT", "restartScript");
  return o;
}
...
const ConfigSchema = z.object({
  ...
  restartScript: z.string().default(DEFAULTS.restartScript),
...
function apply(ctx, config = {}) {
  ...
  const log = (...a) => console.log(new Date().toISOString(), "[tg-bridge]", ...a);
