  ...
  */
  ...
  async handleModelCommand(chat, arg) {
    ...
    try {
      ...
      let res;
      try {
        res = await this.dsh("session.selectModel", payload);
      } catch (e) {
        // Double safety: a catalog may over-report effort support; retry the
        // switch without the effort so the model still changes.
        if (String(e.message).includes("does not support reasoning effort") && payload.reasoningEffort !== undefined) {
          this.log("model switch retried without reasoning effort:", e.message);
