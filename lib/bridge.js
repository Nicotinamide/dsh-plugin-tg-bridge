...
const I18N = {
  ...
  modelEffortPrompt: { zh: "🎛 {name} 不支持当前推理强度（{cur}）。\n请选择切换后的推理强度：", en: "🎛 {name} does not support the current effort ({cur}).\nPick the effort to switch with:" },
  modelEffortUnset: { zh: "默认（不设）", en: "Default (unset)" },
  modelEffortUnsupported: { zh: "本模型不支持推理强度", en: "this model does not support reasoning effort" },
  ...
  */
  ...
  async handleModelCommand(chat, arg) {
    ...
    try {
      ...
      // with no reasoning block does not support any effort.
      const supportedEfforts = hit.reasoning?.efforts ?? [];

      // If the current effort is supported, switch directly with it.
      if (curEff && supportedEfforts.some((e) => e.id === curEff)) {
        return await this.performModelSwitch(chat, sid, hit, curEff);
      }

      // Otherwise the target does not match the current effort: ask the user
      // to choose, instead of silently dropping or retrying.
      const rpcId = randomUUID();
      const choice = { chatId: chat, sessionId: sid, provider: hit.provider, model: hit.model, name: hit.name, efforts: supportedEfforts, chosen: null };
      this.pendingEffortChoice.set(rpcId, choice);
      const buttons = supportedEfforts.map((e) => [{ text: e.id, callback_data: `effopt:${rpcId}:${e.id}` }]);
      // Always offer "default/unset" as an explicit choice.
      buttons.push([{ text: this.t(chat, "modelEffortUnset"), callback_data: `effopt:${rpcId}:` }]);
      await this.tg.call("sendMessage", {
        chat_id: chat,
        text: this.t(chat, "modelEffortPrompt", { name: hit.name, cur: curEff ?? this.t(chat, "effortUnset") })
          + (supportedEfforts.length ? "" : `\n(${this.t(chat, "modelEffortUnsupported")})`),
  ...
  /** Perform the actual selectModel switch for a resolved model + effort. */
  ...
  */
  ...
  async handleCallback(cq) {
    ...
    if (data === "allow" || data === "reject") {
      ...
      // Format: effopt:<rpcId>:<effort>. Empty effort = "default (unset)".
      ...
      if (!choice) {
        ...
        if (choice.chatId === chat) {
          await this.performModelSwitch(chat, choice.sessionId, { provider: choice.provider, model: choice.model, name: choice.name }, eff || undefined);
          responded = true;
        } else {
          this.log("effort choice denied for non-initiator:", cq.from?.id, "chat", chat, "expected", choice.chatId);
          alertText = this.t(chat, "deniedQuestion");
          responded = true;
        }
      }
