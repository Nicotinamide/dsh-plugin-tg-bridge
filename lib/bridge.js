...
export class Bridge {
  ...
  attachHost({ permissionPresets, sessionService, settingsService } = {}) {
    ...
    // same settings namespace the GUI edits — one source of truth.
    if (settingsService !== undefined) this.settingsService = settingsService;
  }
  ...
  */
  ...
  async persistAccess(chat, nextAllowedUsers, nextAdminChatIds) {
    if (!this.settingsService) {
      await this.send(chat, "❌ 设置服务不可用，无法持久化授权。请改配置文件里的 allowedUsers/adminChatIds 后重启。");
      return false;
    }
    try {
  ...
  async handleUsersCommand(chat) {
    ...
    // Enrich each row with the real Telegram display name (private chat:
    // @username or first name; group: title). The configured label wins for
    // display with the real name as a hint; without a label the real name
    // replaces the bare id. getChat fails quietly (e.g. bot not in that group).
    const rows = await Promise.all([...map.entries()].map(async ([id, e]) => {
      let real = null;
      try {
        const info = await this.tg.call("getChat", { chat_id: id });
        real = info.type === "private"
          ? (info.username ? `@${info.username}` : (info.first_name ?? null))
          : (info.title ?? null);
      } catch {}
      const name = e.label ? (real && real !== e.label ? `${e.label}（${real}）` : e.label) : (real ?? id);
      return `${e.admin ? "🛡" : "👤"} ${id}  ${name}`;
    }));
    await this.send(chat, `📋 授权列表（${rows.length}）:\n\n${rows.join("\n") || "（空）"}\n\n/grant <chatId> [label] 添加\n/revoke <chatId> 移除\n/admin [off] <chatId> 管理管理员\n/label <chatId> <名称> 设置显示名`);
  }

  /** Admin: set or clear a chat's display label (persisted to settings). */
  async handleLabelCommand(chat, arg) {
    if (!this.isAdmin(chat)) { await this.send(chat, "⛔ 仅管理员可执行 /label。"); return; }
    const m = String(arg ?? "").trim().match(/^(-?\d+)(?:\s+(.*))?$/);
    if (!m) { await this.send(chat, "用法: /label <chatId> <名称>（不写名称则清除 label）"); return; }
    const id = m[1];
    const newLabel = m[2]?.trim() || undefined;
    const users = [...(this.config.allowedUsers ?? [])];
    const found = users.find((u) => String(u.chatId) === id);
    if (!found) {
      await this.send(chat, `ℹ️ ${id} 不在授权列表，先 /grant。`);
      return;
    }
    if (newLabel === undefined) delete found.label; else found.label = newLabel;
    if (!await this.persistAccess(chat, users, [...this.adminChats])) return;
    this.applyAccessConfig({ ...this.config, allowedUsers: users });
    await this.send(chat, newLabel ? `✅ 已设置 ${id} 的显示名: ${newLabel}` : `✅ 已清除 ${id} 的显示名。`);
  }
  ...
  async handleGrantCommand(chat, arg) {
    ...
    if (!target) {
      ...
      else { await this.send(chat, "用法: /grant <chatId> [label]\n在群里直接 /grant 可授权当前群。"); return; }
    }
    const m = target.match(/^(-?\d+)(?:\s+(.*))?$/);
    if (!m) { await this.send(chat, `❌ 无法解析 chatId: ${target}`); return; }
    const id = m[1];
    let label = m[2]?.trim() || undefined;
    if (!label && String(id).startsWith("-")) {
      try { const info = await this.tg.call("getChat", { chat_id: id }); label = info.title ?? undefined; } catch {}
    }
    const users = [...(this.config.allowedUsers ?? [])];
    if (users.some((u) => String(u.chatId) === id)) {
      await this.send(chat, `ℹ️ ${id} 已在授权列表${label ? "（label: " + label + "）" : ""}。`);
      return;
    }
  ...
  */
  ...
  async processUpdate(u) {
    ...
    if (cmd === "/help") {
      ...
      if (admin) lines.push(
        "/users — 授权列表",
        "/grant <chatId> [label] — 添加用户/群组（群里直接 /grant 授权当前群）",
        "/revoke <chatId> — 移除授权",
        "/admin [off] <chatId> — 设置/取消管理员",
        "/label <chatId> <名称> — 设置/清除显示名",
        "/restart — 重启 DSH web",
      );
    ...
    if (cmd.startsWith("/label")) { try { await this.handleLabelCommand(chat, cmd.slice(6)); } catch (e) { err(e); } return; }
