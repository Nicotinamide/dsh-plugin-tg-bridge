...
const I18N = {
  ...
  modeLockedHint: { zh: "⚠️ 当前会话已开始，模式已固定（DSH 限制）。换模式请 /use new 新建会话后选择。", en: "⚠️ This session has already started; its mode is fixed. Use /use new to create a session, then pick a mode." },
  modeLocked: { zh: "当前会话已开始，模式已固定（DSH 限制）。想换模式：/use new 新建会话，或 /use 切换到空会话后再 /mode。", en: "This session has already started; its mode is fixed. To change mode: /use new to create a session, or switch to a blank session with /use, then /mode." },
...
export class Bridge {
  ...
  *  cordis...), Chinese users see the configured name (标准模式...). */
  ...
  async handleModeCommand(chat, arg) {
    ...
    const sess = sessionsRes?.items?.find((s) => s.sessionId === sid);
    const current = sess?.agentPreset;
    const target = String(arg ?? "").trim();
    ...
    if (target) {
      ...
      if (preset.id === current) { await this.send(chat, this.t(chat, "modeAlready", { name: this.presetLabel(chat, preset) })); return; }
      try {
        await this.dsh("agentPreset.select", { sessionId: sid, agentPreset: preset.id });
        await this.send(chat, this.t(chat, "modeSwitched", { name: this.presetLabel(chat, preset), id: preset.id }));
      } catch (e) {
        await this.send(chat, String(e.message).includes("agent-preset-locked") ? this.t(chat, "modeLocked") : this.t(chat, "modeFailed", { err: e.message }));
      }
      return;
    }
    // Button mode: one button per preset, current one marked.
    const curPreset = presets.find((p) => p.id === current);
    const title = this.t(chat, "modeButtonTitle", { name: this.presetLabel(chat, curPreset ?? { id: current ?? "?" }) })
      + (sess && !sess.blank ? "\n\n" + this.t(chat, "modeLockedHint") : "");
    const keyboard = presets.map((p) => [{
      text: (p.id === current ? "✅ " : "") + this.presetLabel(chat, p),
    ...
    if (data === "allow" || data === "reject") {
      ...
      if (!sid) {
        ...
        try {
          ...
          this.modeReply = `${this.presetLabel(chat, preset ?? { id: presetId })}（${res?.agentPreset ?? presetId}）`;
          responded = true;
        } catch (e) {
          this.log("mode switch err:", e.message);
          this.modeReply = String(e.message).includes("agent-preset-locked") ? this.t(chat, "modeLocked") : `❌ ${e.message}`;
          responded = true;
        }
      }
    }
