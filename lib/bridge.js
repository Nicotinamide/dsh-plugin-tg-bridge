...
/**
* Resolve where this process's stdout points (e.g. dsh-web.log), so the
* restart watchdog can redirect the relaunched process to the same log file.
* Falls back to the conventional DSH home log when stdout is /dev/null or not
* a file, so a log-less relaunch still lands somewhere greppable.
*/
...
export class Bridge {
  ...
  log(...a) {
    console.log(new Date().toISOString(), "[tg-bridge]", ...a);
  }
  ...
  /**
  * Restart dsh web without any external script: spawn a detached watchdog that
  * waits for this process to exit, then relaunches dsh from the exact argv this
  * process was started with. Zero config, shareable out of the box.
  */
  async handleRestartCommand() {
    ...
    this.save();
    try {
      // The watchdog: wait for our pid to disappear, then exec the same command.
      // Single quotes in argv are escaped so the shell command survives.
      // stdout/stderr are redirected to wherever THIS process logs (its own
      // fd 1 target), so the relaunched dsh keeps writing the same log file.
      const argv = process.argv;
      const cmd = argv.map((a) => `'${String(a).replace(/'/g, `'\\''`)}'`).join(" ");
      const logTarget = stdoutTarget();
      const redir = logTarget ? ` >> '${logTarget}' 2>&1` : " >/dev/null 2>&1";
      const watchdog = `while kill -0 ${process.pid} 2>/dev/null; do sleep 0.2; done; exec ${cmd}${redir}`;
      ...
      // (bash missing) never leaves a stale marker.
      try { await this.send(this.chat, "🔄 正在重启 DSH web…（约 10 秒后自动汇报状态）"); } catch {}
      // Exit this process so the watchdog's kill -0 loop ends and it relaunches.
