...
export class Bridge {
  ...
  /**
  * Restart dsh web by spawning the restart script detached. The script kills
  * this process and starts a fresh one; being detached + unref'd, it survives
  * our own exit. Runs inside the dsh web process, so it has full permissions —
  * unlike spawning from an external sandbox.
  */
  async handleRestartCommand() {
    const script = this.config.restartScript;
    if (!script) {
      await this.send(this.chat, "❌ 未配置 restartScript（通过 patch 或 TG_RESTART_SCRIPT 设置重启脚本路径）。");
      return;
    }
    ...
    try {
      ...
      child.unref();
      this.log("restart script spawned:", script);
      // Acknowledge only after the restart is actually underway, so a failed
      // spawn (TG unreachable or script missing) never leaves a stale marker.
