          ...
          switch (ev.type) {
            case "turn/start":
              ...
              if (ours) {
                ...
                // Visible "processing" placeholder. Groups do not render a
                // bot's typing indicator, so this message is the only progress
                // hint there; it becomes the tool-progress message when tools
                // run, and is deleted at turn/end.
                try {
                  const m = await this.tg.call("sendMessage", { chat_id: chat, text: this.t(chat, "processing") });
