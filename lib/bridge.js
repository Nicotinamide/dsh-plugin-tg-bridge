...
export class Bridge {
  ...
  * commands right after they persist to settings.
  */
  applyAccessConfig(cfg) {
    this.config = { ...this.config, ...cfg };
    const chats = new Set([...(cfg.allowedUsers ?? []).map((u) => String(u.chatId))]);
    if (cfg.allowedChat && cfg.allowedChat !== "") chats.add(String(cfg.allowedChat));
    this.chats = chats;
    this.adminChats = new Set((cfg.adminChatIds ?? []).map((c) => String(c)));
    this.chatToLabel = new Map((cfg.allowedUsers ?? []).map((u) => [String(u.chatId), u.label ?? String(u.chatId)]));
  ...
  */
  ...
  async handleUsersCommand(chat) {
    ...
    const rows = [...map.entries()].map(([id, e]) => `${e.admin ? "🛡" : "👤"} ${id}${e.label ? "  " + e.label : ""}`);
  ...
  */
  ...
  async processUpdate(u) {
    ...
    if (cmd.startsWith("/use")) { try { await this.handleUseCommand(chat, cmd.slice(4)); } catch (e) { err(e); } return; }
