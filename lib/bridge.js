...
const I18N = {
  ...
  sessionsHint: { zh: "用 /use <编号|ID|标题> 切换，/mode 切模式，/rename <新标题> 重命名，/use new 新建。", en: "/use <num|ID|title> to switch, /mode to change mode, /rename <title> to rename, /use new to create." },
  ...
  modeNoList: { zh: "❌ 无法获取模式列表。", en: "❌ Could not fetch the mode list." },
  modeNotFound: { zh: "❌ 找不到模式 {target}。用 /mode 查看按钮。", en: "❌ No mode {target}. Use /mode for buttons." },
  modeAlready: { zh: "ℹ️ 当前会话已是 {name}。", en: "ℹ️ Current session is already {name}." },
  modeSwitched: { zh: "✅ 已切换到 {name}（{id}），新一轮生效。", en: "✅ Switched to {name} ({id}); takes effect next turn." },
  modeFailed: { zh: "❌ 切换失败: {err}", en: "❌ Switch failed: {err}" },
  modeButtonTitle: { zh: "🎛 当前会话模式: {name}\n\n点按钮切换（新一轮生效）：", en: "🎛 Current mode: {name}\n\nTap a button to switch (takes effect next turn):" },
  modeReply: { zh: "✅ 已切换到模式 {reply}", en: "✅ Switched to mode {reply}" },
  ...
  langSet: { zh: "✅ 语言已设为 {lang}（手动指定，覆盖 Telegram 上报）。", en: "✅ Language set to {lang} (manual override)." },
  langStatus: { zh: "🌐 当前语言: {current}（{detected}）", en: "🌐 Current language: {current} ({detected})" },
  langManual: { zh: "手动指定", en: "manual override" },
  langAutoNone: { zh: "Telegram 未上报，默认中文", en: "no Telegram language, default zh" },
  langName: { zh: "中文", en: "English" },
  langUsage: { zh: "用法: /lang <zh|en|auto>\nzh=中文，en=English，auto=跟随 Telegram 语言", en: "Usage: /lang <zh|en|auto>\nzh=Chinese, en=English, auto=follow Telegram" },
};
...
export class Bridge {
  ...
  */
  ...
  /** Chat language: "en" for English, "zh" otherwise. An explicit /lang
  *  override wins; otherwise Telegram's reported from.language_code decides. */
  lang(chat) {
    const r = this.runtime(chat);
    if (r.langOverride) return String(r.langOverride).toLowerCase().startsWith("en") ? "en" : "zh";
    const l = String(r.language ?? "").toLowerCase();
    return l.startsWith("en") ? "en" : "zh";
  }

  /** Manual language switch: /lang <zh|en|auto>. */
  async handleLangCommand(chat, arg) {
    const target = String(arg ?? "").trim().toLowerCase();
    const r = this.runtime(chat);
    if (target === "en" || target === "zh") {
      r.langOverride = target;
      await this.send(chat, this.t(chat, "langSet", { lang: target === "en" ? "English" : "中文" }));
      return;
    }
    if (target === "auto" || target === "") {
      if (target === "auto") delete r.langOverride;
      const current = this.t(chat, "langName");
      const detected = r.langOverride
        ? this.t(chat, "langManual")
        : (r.language ? `${r.language} → ${this.t(chat, "langName")}` : this.t(chat, "langAutoNone"));
      await this.send(chat, this.t(chat, "langStatus", { current, detected }));
      return;
    }
    await this.send(chat, this.t(chat, "langUsage"));
  }
  ...
  */
  ...
  async handleSessionsCommand(chat) {
    ...
    const lines = visible.map((s, i) => {
      ...
      const preset = s.agentPreset ? `[${presetName.get(s.agentPreset) ?? s.agentPreset}]` : "";
      let src = "";
      if (all) {
  ...
  /** Mode (agent preset) management: show the current mode as switch buttons (or /mode <id> direct). */
  async handleModeCommand(chat, arg) {
    const sid = this.chatEntry(chat).current ?? (await this.ensureSession(chat));
    ...
    if (presets.length === 0) { await this.send(chat, this.t(chat, "modeNoList")); return; }
    const current = sessionsRes?.items?.find((s) => s.sessionId === sid)?.agentPreset;
    const target = String(arg ?? "").trim();
    // Direct switch: /mode <number|id>
    if (target) {
      let preset = null;
      if (/^\d+$/.test(target)) preset = presets[parseInt(target, 10) - 1] ?? null;
      else preset = presets.find((p) => p.id === target || (p.name ?? "").includes(target)) ?? null;
      if (!preset) { await this.send(chat, this.t(chat, "modeNotFound", { target })); return; }
      if (preset.id === current) { await this.send(chat, this.t(chat, "modeAlready", { name: preset.name ?? preset.id })); return; }
      try {
        await this.dsh("agentPreset.select", { sessionId: sid, agentPreset: preset.id });
        await this.send(chat, this.t(chat, "modeSwitched", { name: preset.name ?? preset.id, id: preset.id }));
      } catch (e) { await this.send(chat, this.t(chat, "modeFailed", { err: e.message })); }
      return;
    }
    // Button mode: one button per preset, current one marked.
    const curName = presets.find((p) => p.id === current)?.name ?? current ?? "?";
    const keyboard = presets.map((p) => [{
      text: (p.id === current ? "✅ " : "") + (p.name ?? p.id),
      callback_data: `mode:${p.id}`,
    }]);
    await this.tg.call("sendMessage", {
      chat_id: chat,
      text: this.t(chat, "modeButtonTitle", { name: curName }),
      reply_markup: JSON.stringify({ inline_keyboard: keyboard }),
    });
  }
  ...
  */
  ...
  async handleCallback(cq) {
    ...
    if (data === "allow" || data === "reject") {
      ...
      const presetId = data.slice(5);
      const chat = String(cq.message?.chat?.id ?? "");
      const sid = this.chatEntry(chat).current ?? (chat ? await this.ensureSession(chat) : null);
      if (!sid) {
        this.log("mode switch failed: no chat/session");
      } else {
        try {
          const presetsRes = await this.dsh("agentPreset.list", {}).catch(() => null);
          const name = (presetsRes?.presets ?? []).find((p) => p.id === presetId)?.name ?? presetId;
          const res = await this.dsh("agentPreset.select", { sessionId: sid, agentPreset: presetId });
          this.log("mode switched to:", presetId);
          this.modeReply = `${name}（${res?.agentPreset ?? presetId}）`;
          responded = true;
        } catch (e) {
          this.log("mode switch err:", e.message);
          this.modeReply = `❌ ${e.message}`;
          responded = true;
        }
      }
    }
    ...
    if (responded && !denied) {
      try {
        if (this.permissionReply !== undefined) {
          ...
          this.effortReply = undefined;
        } else if (this.modeReply !== undefined) {
          await this.tg.call("editMessageText", {
            chat_id: replyChat, message_id: tgMsgId,
            text: this.t(replyChat, "modeReply", { reply: this.modeReply }),
          });
          this.modeReply = undefined;
        } else {
  ...
  async processUpdate(u) {
    ...
    if (langCode) this.runtime(chat).language = langCode;
    ...
    // /mode must be exact: "/model 1" starts with "/mode", but /model above
    // already claimed it; this branch only sees /mode and /mode <arg>.
    if (/^\/mode(?:\s|$)/.test(cmd)) { try { await this.handleModeCommand(chat, cmd.slice(5)); } catch (e) { err(e); } return; }
    if (cmd.startsWith("/rename")) { try { await this.handleRenameCommand(chat, cmd.slice(7)); } catch (e) { err(e); } return; }
    if (cmd.startsWith("/lang")) { try { await this.handleLangCommand(chat, cmd.slice(5)); } catch (e) { err(e); } return; }
