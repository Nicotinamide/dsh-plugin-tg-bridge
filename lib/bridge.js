...
const I18N = {
  ...
  modelEffortPrompt: { zh: "🎛 请选择 {name} 的思考强度：", en: "🎛 Pick the reasoning effort for {name}:" },
  ...
  stModel: { zh: "📚 模型: {m}", en: "📚 Model: {m}" },
  stMode: { zh: "🎛 模式: {m}", en: "🎛 Mode: {m}" },
  ...
  groupCmdDenied: { zh: "🔒 群组里只支持提问和查看状态。请私聊使用 {cmd}。", en: "🔒 Groups only support asking questions and viewing status. Use {cmd} in a private chat." },
};
...
export class Bridge {
  constructor(config) {
    ...
    this.pendingReplies = new Map();  // sessionId -> count of queued TG-initiated turns awaiting reply
    this.permissionReply = undefined;
    this.effortReply = undefined;
    this.newmodeReply = undefined;
    this.effoptReply = undefined;
    // Pending model-switch choices awaiting the user to pick a reasoning
    // effort (shown when the target model does not support the current one).
    // Transient only: not persisted, so after a restart the old buttons answer
    // with choiceExpired — re-run /model.
    // rpcId -> { chatId, sessionId, provider, model, name, efforts, chosen }
    this.pendingEffortChoice = new Map();
  ...
  *  /restart replies in the user's language before any new message arrives. */
  ...
  async dsh(method, payload) {
    ...
    try {
      ...
      // Retry once on transient network failures (the API is loopback here, but
      // flaky proxies/TLS can still produce fetch failed).
  ...
  */
  ...
  startTyping(chat) {
    ...
    // Fire immediately so even a short turn shows the typing indicator (a
    // plain setInterval would wait 4s and a quick turn would never show it).
    const fire = async () => { try { await this.tg.call("sendChatAction", { chat_id: chat, action: "typing" }); } catch {} };
    fire();
    r.typingTimer = setInterval(fire, 4000);
  }
    ...
    try {
      switch (p.type) {
        case "approval/requested": {
          ...
          if (sid === undefined || (this.pendingReplies.get(sid) ?? 0) <= 0) break;
        ...
        case "session/event": {
          const ev = p.event ?? {};
          const ours = sid !== undefined && (this.pendingReplies.get(sid) ?? 0) > 0;
          switch (ev.type) {
            case "turn/start":
  ...
  async handleModelCommand(chat, arg, fromId) {
    ...
    try {
      ...
      // Determine which reasoning efforts the target model supports. A model
      // with no reasoning block does not support any effort at all.
      const supportedEfforts = hit.reasoning?.efforts ?? [];

      // No reasoning support: switch directly (there is nothing to choose).
      if (supportedEfforts.length === 0) {
        const text = await this.performModelSwitch(chat, sid, hit, undefined);
        await this.send(chat, text);
        return;
      }

      // The model supports reasoning: always ask the user to pick the effort
      // explicitly (marking the current one if still available), so the switch
      // never silently lands on "unset".
  ...
  async buildStatusText(chat) {
    ...
    if (sid !== null) {
      try {
        ...
        const found = items.find((s) => s.sessionId === sid);
        // Model + mode summary right after the session line.
  ...
  */
  ...
  async handleMessage(chat, text, quoted, fromId) {
    ...
    // Start the typing indicator immediately — from the moment the message
    // arrives — so it shows until turn/end stops it. turn/start re-fires it
    // (idempotent), and stopTyping on turn/end always ends it.
    this.startTyping(chat);
    const r = this.runtime(chat);
    ...
    if (fromId !== undefined && (this.pendingReplies.get(sid) ?? 0) <= 0) r.askerId = fromId;
    // Mark this session as owing a TG reply BEFORE prompting: turn/start can
    // arrive on the mux while session.prompt is still awaiting, and if the
    // marker is added only afterwards that first turn/start is seen as a
    // web-started turn (ours=false) — no typing indicator, though the reply
    // still arrives. Rolling back on prompt failure keeps the count honest.
    ...
    try {
      ...
      if (c <= 0) this.pendingReplies.delete(sid); else this.pendingReplies.set(sid, c);
      this.save();
      // No turn will run (and no turn/end will stop the indicator).
      this.stopTyping(chat);
      throw e;
    }
    // Non-blocking timeout safety: replies arrive via mux turn/end; if the
    // turn never ends, notify once and stop the typing indicator. Never
  ...
  */
  ...
  async handleCallback(cq) {
    ...
    // The message path checks isAllowed before dispatch; callbacks must too, so
    // a revoked chat cannot keep answering approvals or switching models.
    if (chat && !this.isAllowed(chat)) {
      this.log("callback ignored from unauthorized chat:", chat);
      try { await this.tg.call("answerCallbackQuery", { callback_query_id: cq.id, text: this.t(chat, "deniedChat") }); } catch {}
      return;
    }
    ...
    if (data === "allow" || data === "reject") {
      ...
      if (!choice) {
        ...
        alertText = this.t(chat, "choiceExpired");
        responded = true;
      } else if (choice.clickerId !== undefined && cq.from?.id !== choice.clickerId) {
        // Only the person who ran /model may pick the effort.
        ...
        this.effoptReply = await this.performModelSwitch(chat, choice.sessionId, { provider: choice.provider, model: choice.model, name: choice.name }, eff || undefined);
        responded = true;
      }
    ...
    if (responded && !denied) {
      // Resolve the reply text first, then clear ALL reply fields before the
      // edit: a failed editMessageText must not leak a stale reply into the
      // next callback (single-instance fields shared across chats).
      let replyText = null;
      if (this.permissionReply !== undefined) replyText = this.t(replyChat, "permReply", { name: this.permissionReply });
      else if (this.effortReply !== undefined) replyText = this.t(replyChat, "effortReply", { name: this.effortReply });
      else if (this.effoptReply !== undefined) replyText = this.effoptReply;
      else if (this.newmodeReply !== undefined) replyText = this.t(replyChat, "useNewCreated", { mode: this.newmodeReply.name, sid: this.newmodeReply.sid });
      this.permissionReply = this.effortReply = this.effoptReply = this.newmodeReply = undefined;
      try {
  ...
  async processUpdate(u) {
    ...
    } else if (this.runtime(chat).language === undefined && this.state.chatLangs?.[String(chat)] === undefined) {
      // No language signal anywhere: default to zh, but never shadow a
      // persisted language (a message without language_code must not flip an
      // English chat back to Chinese).
    ...
    // Groups are question-only: plain messages (sent to the agent) plus a tiny
    // read-only command set. Everything else needs a private chat, so other
    // members cannot change the shared group session.
    ...
    if (cmd === "/help") {
      ...
      // Groups get a trimmed surface: only asking + read-only commands exist.
    ...
    if (cmd.startsWith("/model")) { try { await this.handleModelCommand(chat, cmd.slice(6), msg.from?.id); } catch (e) { err(e); } return; }
  ...
  start() {
    ...
    // on the SAME instance, and start() re-arms stopped=false. Without
    // clearing a previous start's timers, TWO delayed pollLoop/connectMux
    // callbacks would fire (both see stopped=false) => two getUpdates pollers
    // => permanent 409 Conflict. Clear any pending timers first.
    if (this.pollTimer) clearTimeout(this.pollTimer);
    if (this.muxTimer) clearTimeout(this.muxTimer);
    // Delay polling and the mux until the instance has stabilised: a dying
    // instance must not claim Telegram updates or a mux connection, or its
    // async processUpdate would still run and duplicate replies (e.g. two
    // identical "restart refused" messages from two instances of the same
    // update).
  ...
  stop() {
    if (this.stopped) return;
    this.stopped = true;
    // Cancel any not-yet-fired delayed start timers (double poller guard).
    if (this.pollTimer) { clearTimeout(this.pollTimer); this.pollTimer = undefined; }
    if (this.muxTimer) { clearTimeout(this.muxTimer); this.muxTimer = undefined; }
