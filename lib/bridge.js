...
export class Bridge {
  ...
  */
  ...
  /** Mode (agent preset) management: list or switch the current session's preset. */
  async handleModeCommand(chat, arg) {
    ...
    if (!target) {
      const lines = presets.map((p, i) => {
        const mark = p.id === current ? "✅" : `${i + 1}`;
        const def = p.isDefault ? "（默认）" : "";
        return `${mark}. ${p.name ?? p.id}${def} — ${p.id}${p.description ? "\n   " + p.description.slice(0, 70) : ""}`;
      });
      await this.send(chat, `🎛 模式列表（当前会话: ${presets.find((p) => p.id === current)?.name ?? current ?? "?"}）:\n\n${lines.join("\n")}\n\n用 /mode <编号|id> 切换当前会话模式（新一轮生效）。`);
      return;
    }
    let preset = null;
    ...
    if (!preset) { await this.send(chat, `❌ 找不到模式 ${target}。用 /mode 查看列表。`); return; }
