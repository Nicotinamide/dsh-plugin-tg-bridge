...
export class Bridge {
  ...
  */
  ...
  async handleSessionsCommand(chat) {
    ...
    const lines = visible.map((s, i) => {
      ...
      if (all) {
        ...
        src = owner ? `[TG ${this.chatToLabel.get(owner) ?? owner}]` : "[Web]";
      }
      return `${mark}. ${label}${run}${blank}${preset}${src ? " " + src : ""}\n   ${s.sessionId}`;
    });
    const owner = all ? "\n（管理员视图：DSH 全部会话，含 Web 端创建的）" : "";
    await this.send(chat, `📋 会话列表（共 ${visible.length} 个）:${owner}\n\n${lines.join("\n")}\n\n用 /use <编号|ID|标题> 切换，/mode 切模式，/rename <新标题> 重命名，/use new 新建。`);
  }
  ...
  async handleModeCommand(chat, arg) {
    ...
    if (presets.length === 0) { await this.send(chat, "❌ 无法获取模式列表。"); return; }
    ...
    if (target) {
      ...
      if (!preset) { await this.send(chat, `❌ 找不到模式 ${target}。用 /mode 查看按钮。`); return; }
      if (preset.id === current) { await this.send(chat, `ℹ️ 当前会话已是 ${preset.name ?? preset.id}。`); return; }
      try {
        await this.dsh("agentPreset.select", { sessionId: sid, agentPreset: preset.id });
        await this.send(chat, `✅ 已切换到 ${preset.name ?? preset.id}（${preset.id}），新一轮生效。`);
      } catch (e) { await this.send(chat, `❌ 切换失败: ${e.message}`); }
      return;
    }
  ...
  async handleRenameCommand(chat, arg) {
    const raw = String(arg ?? "").trim();
    if (!raw) { await this.send(chat, "用法: /rename <新标题>（重命名当前会话）"); return; }
    const sid = this.chatEntry(chat).current ?? (await this.ensureSession(chat));
    try {
      const res = await this.dsh("session.rename", { sessionId: sid, title: raw });
      await this.send(chat, `✅ 已重命名为: ${res.title ?? raw}`);
    } catch (e) {
      await this.send(chat, `❌ 重命名失败: ${e.message}`);
    }
  }
  ...
  */
  ...
  async persistAccess(chat, nextAllowedUsers, nextAdminChatIds) {
    if (!this.settingsService) {
      ...
      await this.send(chat, "❌ 设置服务尚未就绪，请稍后重试，或 /restart 后重试。");
      return false;
    }
    try {
